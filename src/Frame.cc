...
Frame::Frame(): mpcpi(NULL), mpPrevFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbIsSet(false), mbImuPreintegrated(false), mbHasPose(false), mbHasVelocity(false)
{
...
//      mpPrevFrame(pPrevF), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbIsSet(false), mbImuPreintegrated(false),
...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     ...
     mpPrevFrame(pPrevF), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbIsSet(false), mbImuPreintegrated(false), mpCamera(pCamera),
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    cv::drawKeypoints(imGray, mvKeys, im_with_keypoints, cv::Scalar(0,0,255), cv::DrawMatchesFlags::DEFAULT);
    cv::imshow("keypoints", im_with_keypoints);
    cv::waitKey(0);

    N = mvKeys.size();
    cout << "Number of keypoints: " << N << endl;
    if(mvKeys.empty())
        return;

    UndistortKeyPoints();
    // undistort the image
    cv::Mat imGrayUndistorted;
    cv::undistort(imGray, imGrayUndistorted, mK, mDistCoef);
    // draw the keypoints on the undistorted image
    cv::Mat im_with_keypoints_undistorted;
    cv::drawKeypoints(imGrayUndistorted, mvKeysUn, im_with_keypoints_undistorted, cv::Scalar(0,0,255), cv::DrawMatchesFlags::DEFAULT);
    cv::imshow("keypoints undistorted", im_with_keypoints_undistorted);
    cv::waitKey(0);
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera, GeometricCamera* pCamera2, Sophus::SE3f& Tlr,Frame* pPrevF)
        :mpcpi(NULL), mpORBvocabulary(voc),mpORBextractorLeft(extractorLeft),mpORBextractorRight(extractorRight), mTimeStamp(timeStamp), mK(K.clone()), mK_(Converter::toMatrix3f(K)),  mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
         mpPrevFrame(pPrevF), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbImuPreintegrated(false), mpCamera(pCamera), mpCamera2(pCamera2),
